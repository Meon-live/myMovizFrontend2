...
function Home() {
 ...
 useEffect(() => {
   fetch('https://my-moviz-backend-alpha.vercel.app/movies')
     ...
     .then(data => { 
      const addMovies = data.allMovies.results.map(addMovie => ({
        ...
        voteCount: addMovie.vote_count,
        overview: addMovie.overview.length > 250 ? addMovie.overview.substring(0, 250) + '...' : addMovie.overview
      }));
      setMoviesData(addMovies);
    })
 }, []);
